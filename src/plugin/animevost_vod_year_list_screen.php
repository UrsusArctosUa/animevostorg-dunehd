...
class AnimevostVodYearListScreen extends AbstractPreloadedRegularScreen {

    const ID = 'vod_year_list';
...
//                    ViewItemParams::item_detailed_icon_path => $year->get_icon_path()
    ...
    private function fetch_list() {
        ...
        $rawHtml = file_get_contents(AnimevostConfig::VOD_YEAR_LIST_URL_FORMAT, null, $context);
//        $rawHtml    = HD::http_get_document(sprintf(AnimevostConfig::VOD_YEAR_LIST_URL_FORMAT,
//                $param, $this->get_page_for_index($from_ndx, 10)));
        $html    = str_get_html($rawHtml);

        if (is_null($html))
            throw new Exception('Can not fetch year list');

        $list = array();

        foreach ($html->find('ul#topnav a[href$=/god/]', 0)->parent()->find('span a') as $element) {
            $item            = array();
